...
It's not perfect. The problem we had, as we will discuss later, is that it 
is really hard to estimate these parameters $a$ and $b$ accurately. It takes 
...
two sampling functions $\Sm_{min}(X), \Sm_{uni}(X)$ with the error map
of the default sampling function $\Sm_{default}(X)$ which always outputs $\bar m$.
...
the error with the default sampling function $\Sm_{default}$.
...
\par This gives us a general insight: \textit{uniform error
 and minimising the mean error require two sampling maps that 
 are really different}. In other words, to minimise the 
 mean error, you don't want to have the same error in 
 every pixel.

\subsection{Prediction of the parameter $a$ and $b$} \label{ab}

As said before, the estimation of $a$ and $b$ is a difficult problem.
 Can we predict these parameters with a minimum of information that is fast 
 to compute? 

\par From a dataset containing many scene $X$ pairs 
(input=$X_{\bar m}$,output=$(a,b)$) with a small $m$, train a convolutional 
neural network that outputs $a$ and $b$ from $X_{\bar m}$ with auxiliary buffers.

\paragraph*{Discussion} There is a big problem with this idea, it is probably 
not true that we can find a strong enough link between low sample ($X_{\bar m}$) 
images and their respective convergence rates ($a$ and $b$). We clearly have 
a huge lack of information. Our hope is that, as in rendering, neighbours 
pixel integrale are similar, the network has more information than just 
the $m$ samples of the pixel to predict $a$ and $b$. This is the reason 
why we use a CNN for the network, to maximise the chance of finding a 
connection between neighbouring pixels.

\subsubsection{Create a dataset}

\par The first difficulty was to create a dataset on which to train the network.

As input to our network we have the low sample count image $X_{\bar m}$ with auxiliary 
buffers, which I will explain later, and as output we have the two parameter 
maps $a$ and $b$. So all we had to do was find the truth parameters $a$ 
and $b$ for each scene.
...
Finally, I did a linear regression in log space to find the 
parameter, since we can rewrite the equation
\ref{eq:99}:
\[ \log(e_p) = -a_p\times \log(n_p) + \log(b_p)\]
Here is the result of the regression for a typical pixel of an image
\begin{figure}[H]
    \centering
...
computing resources to compute more images to get a 
...
I generated for 150 Go of rendering, it took more than a week on a 32 CPU core. 
And the final size of the dataset is less than 1Go, it only contains $a$ and $b$
 for each scene.  In retrospect, we should have generated a smaller image, perhaps 
 64x64, but with higher quality (more realisations per sample count).
...
\section{Combination of statistic} \label{combi}
...
\section{Image}\label{denoiserImage}
