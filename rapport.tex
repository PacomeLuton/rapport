...
possible as the optimal sampling function. But before, let's looks a the state of the art.
...
\begin{figure}[H]
    ...
    \end{tabular}
\end{figure}
{\color{red} TO DO : comment the result and add another scene}

\paragraph*{Discussion} To conclude this section, we haven't managed 
to make this idea fully work, but the idea is promising. Maybe with more
 data we could get a better result. Or simply, with an image with low 
 spp, we probably on't have enough information to deduce the 
 convergence rate of the error, and we can't get something any better. 
 
\par Finnally, we decide to switch to the next idea, as we thought that
this idea would ultimatly produce worst result than method that directly
optimize to get a good sampling function that work with a denoiser.
